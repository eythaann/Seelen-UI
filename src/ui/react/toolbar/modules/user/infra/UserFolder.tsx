...
import type { UserHomeFolder } from "./UserHome.tsx";
...
export function UserFolder(
  { folderProps, setCategoryOpen, categoryOpen }: UserFolderProps,
) {
  ...
  const OpenOnExplorer = async () => {
    invoke(SeelenCommand.OpenFile, {
      path: await getPathByFolderType(category),
    });
  };

  const onClickChevron = (e: MouseEvent) => {
    e.stopPropagation();
    setCategoryOpen(categoryOpen == category ? FolderType.Unknown : category);
  };

  const files = category == "Recent"
  ...
  return (
    <div className="userhome-directory">
      <div className="userhome-directory-header" onClick={OpenOnExplorer}>
        ...
        <button
          className="userhome-directory-header-collapse-button"
      ...
      >
        ...
        {files.slice(0, folderShowCount).map((item, index) => <FilePreview file={item} key={index} />)}
        {files.length > 5 && (
          <button
            className="userhome-list-extender"
            onClick={(e) => {
              setFolderShowCount(
                files.length > folderShowCount ? folderShowCount * 2 : 5,
              );
              e.stopPropagation();
            }}
          >
            {files.length > folderShowCount ? t("userhome.folders.more_items") : t("userhome.folders.reduce_items")}
          </button>
        )}
      </ul>
    </div>
